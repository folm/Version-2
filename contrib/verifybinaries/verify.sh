...
###   folmcore.org and folm.org and compares them.
...
WORKINGDIR="/tmp/folm_verify_binaries"
TMPFILE="hashes.tmp"

SIGNATUREFILENAME="SHA256SUMS.asc"
RCSUBDIR="test"
HOST1="https://folmcore.org"
HOST2="https://folm.org"
BASEDIR="/bin/"
VERSIONPREFIX="folm-core-"
RCVERSIONSTRING="rc"
...
if [ -n "$1" ]; then
   #let's also check if the version number includes the prefix 'folm-',
...
if [ $? -ne 0 ]; then
   echo "folm.org failed to provide signature file, but folmcore.org did?"
   echo "wget output:"
   echo "$WGETOUT"|sed 's/^/\t/g'
...
if [ "$SIGFILEDIFFS" != "" ]; then
   echo "folm.org and folmcore.org signature files were not equal?"
...
if [ $RET -ne 0 ]; then
   if [ $RET -eq 1 ]; then
      ...
      echo "gpg error. Do you have the Folm Core binary release signing key installed?"
   fi
