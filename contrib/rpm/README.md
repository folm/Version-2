...
The RPM spec file provided here is for Folm-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://folm.org/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[Folm git repository](https://github.com/folm/folm) but are not part of
...
Currently only `Source10` is used, for the example `folm.conf` file.
...
in the folm git repository. At present only an SVG version of the Folm
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the folm
...
needed when 0.12.1 is released, a proper fix is already in the Folm git
master branch.
...
idea to build Folm Core against a static version of that library compiled
...
The `folm.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `folm-server` package. At this point, most RPM based
...
The `folm-server` package should have SELinux support. How to properly do
...
Folm-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/folm/). He has been
packaging Folm for Fedora at least since 2012.
...
4. I renamed the `folm` package that contains the Qt GUI to `folm-core` as
that appears to be how the general population refers to it, in contrast to
`folm-xt` or `folm-classic`. I wanted to make sure the general population
