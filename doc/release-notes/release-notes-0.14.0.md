Folm Core version 0.14.0 is now available from:

  <https://folm.org/bin/folm-core-0.14.0/>
...
Folm Core has supported automatically pruning the blockchain since 0.11. Pruning
...
now ZMQ can be used on Windows. Please see [this document](https://github.com/folm/folm/blob/master/doc/zmq.md) for
...
When Folm Core is out-of-sync on startup, a semi-transparent information
...
Commands sent over the JSON-RPC interface and through the `folm-cli` binary
can now use named arguments. This follows the [JSON-RPC specification](http://www.jsonrpc.org/specification)
for passing parameters by-name with an object.

`folm-cli` has been updated to support this by parsing `name=value` arguments
when the `-named` option is given.

Some examples:

    src/folm-cli -named help command="help"
    src/folm-cli -named getblockhash height=0
    src/folm-cli -named getblock blockhash=000000000019d6689c085ae165831e934ff763ae46a2a6c172b3f1b60a8ce26f
    src/folm-cli -named sendtoaddress address="(snip)" amount="1.0" subtractfeefromamount=true
...
The Alert System was [disabled and deprecated](https://folm.org/en/alert/2016-11-01-alert-retirement) in Folm Core 0.12.1 and removed in 0.13.0. 
...
----------------------
 ...
 - A new RPC command `getmemoryinfo` has been added which will return information
   about the memory usage of Folm Core. This was added in conjunction with
   optimizations to memory management. See [Pull #8753](https://github.com/folm/folm/pull/8753)
...
- #8722 `bb843ad` folm-cli: More detailed error reporting (laanwj)
...
- #7562 `1eef038` Bump transaction version default to 2 (flmdrak)
...
- #8608 `7e9ab95` Install manpages via make install, also add some autogenerated manpages (nomnomflm)
...
- #8568 `078900d` new var `DIST_CONTRIB` adds useful things for packagers from contrib (nomnomflm)
...
- #8830 `ef0801b` test: Add option to run folm-util-test.py manually (jnewbery)
- #8881 `e66cc1d` Add some verbose logging to folm-util-test.py (jnewbery)
...
- #9023 `774db92` Add logging to folm-util-test.py (jnewbery)
- #9065 `c9bdf9a` Merge `doc/unit-tests.md` into `src/test/README.md` (laanwj)
- #9069 `ed64bce` Clean up bctest.py and folm-util-test.py (jnewbery)
...
- #9206 `e662d28` Make test constant consistent with consensus.h (flmdrak)
...
- #9691 `fc67cd2` Init ECC context for `test_folm_fuzzy` (gmaxwell)
...
- #9865 `289204f` Change folm address in RPC help message (marijnfs)
...
- #8813 `fb24d7e` folmd: Daemonize using daemon(3) (laanwj)
...
- #8837 `2108911` Allow folm-tx to parse partial transactions (jnewbery)
- #9204 `74ced54` Clarify CreateTransaction error messages (instagibbs)
- #9265 `31bcc66` folm-cli: Make error message less confusing (laanwj)
...
- #8883 `d5d4ad8` Add all standard TXO types to folm-tx (jnewbery)
